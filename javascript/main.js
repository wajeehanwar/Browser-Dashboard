...
for (let i = 1; i < users.length; i++) {
  if (
    ...
    li.appendChild(
      document.createTextNode(`${users[i].firstName} ${users[i].lastName}`)
    );
...
function addNewUser(event) {
  ...
  for (let i = 0; i < users.length; i++) {
    if (
      nuFirstName.value === users[i].firstName &&
      nuLastName.value === users[i].lastName
    ) {
      userExists = true;
    }
  }
  ...
  if (userExists) {
    ...
    const userId = `${nuFirstName.value}_${
      nuLastName.value
    }_${new Date().getTime()}`;
    users.push(
      new User(
